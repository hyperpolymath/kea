...
// Replace kea, kea, {{DEPS}} with actuals
= kea — Quick Start for Platform Maintainers
:toc:
:toclevels: 2

== Overview

This guide covers packaging, deploying, and maintaining kea for
...
[source,bash]
----
git clone https://github.com/hyperpolymath/kea.git
cd kea
just build-release
----
...
Default config location: `$XDG_CONFIG_HOME/kea/config.toml`

Fallback: `$HOME/.config/kea/config.toml`
...
[source,bash]
----
just install --prefix=/opt/kea-instance1 --config=/etc/kea/instance1.toml
just install --prefix=/opt/kea-instance2 --config=/etc/kea/instance2.toml
----
...
* Upstream: https://github.com/hyperpolymath/kea/issues
