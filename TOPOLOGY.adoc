...
[width="100%",cols="24%,22%,20%,16%,18%",options="header",]
|===
...
|Kea-Mandible CLI |`+mandible/crates/kea-mandible/+` |Rust |80% |Output
formats, help text

|WP-Praxis |`+mandible/crates/wp-praxis/+` |Rust |80% |Multisite, custom
themes

|Slop-Gate |`+mandible/crates/slop-gate/+` |Rust |60% |Heuristic tuning,
thresholds
...
[arabic]
. *Slop-Gate* (60% → 90%): Tune heuristics, add configurable thresholds,
test against real hosting dirs
. *WP-Praxis* (80% → 95%): WordPress multisite support, custom theme
detection
. *Kea-Wit* (70% → 90%): Formalise WIT interfaces for Mandible ↔ Bivouac
pipeline
. *Kea-Mandible CLI* (80% → 95%): JSON/TOML output formats, improved
help text
. *E2E Integration*: Full Mandible → Call → Bivouac pipeline test
. *Container Build*: Chainguard-based OCI image for deployment

=== Phase 2 (Post-MVP)

* Add Idris2 ABI definitions in `+src/abi/+` for cross-component type
safety
